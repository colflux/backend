"""Seed de municipios de Colombia según la División Político-Administrativa (DANE).

Cada municipio se guarda con su código DANE completo de 5 dígitos
(2 del departamento + 3 del municipio). Bogotá D.C. (código DANE 11001)
se asocia al departamento Cundinamarca porque el modelo no maneja a
Bogotá como departamento propio.
"""
...
MUNICIPIOS = {
    ...
    "Cundinamarca": [
        # Bogotá D.C. (DANE 11001) se asocia a Cundinamarca; el modelo no
        # maneja a Bogotá como departamento propio.
        ("11001", "BOGOTA, D.C."),
        ("25001", "AGUA DE DIOS"),
        ("25019", "ALBAN"),
        ("25035", "ANAPOIMA"),
...
def seed(apps, schema_editor):
    ...
    for depto_nombre, municipios in MUNICIPIOS.items():
        ...
        for codigo, nombre in municipios:
            Municipio.objects.update_or_create(
                codigo_dane=codigo,
                defaults={"nombre": nombre, "departamento": departamento},
            )


def unseed(apps, schema_editor):
    Municipio = apps.get_model("app", "Municipio")
    codigos = [c for munis in MUNICIPIOS.values() for c, _ in munis]
    Municipio.objects.filter(codigo_dane__in=codigos).delete()


class Migration(migrations.Migration):

    dependencies = [
        ("app", "0024_alter_monitoreoparcela_options_and_more"),
    ]
